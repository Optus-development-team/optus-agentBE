/**
 * Servicio de Pinata para subir archivos a IPFS.
 *
 * Responsabilidades:
 * - Subir imágenes QR de pago a IPFS
 * - Gestionar archivos multimedia
 * - Generar URLs públicas de IPFS
 *
 * @moved-from src/whatsapp/services/pinata.service.ts
 */
...
export class PinataService {
  ...
  /**
   * Sube datos JSON a Pinata IPFS
   * @param data Datos JSON a subir
   * @param name Nombre para el pin
   * @returns URL pública del JSON en IPFS
   */
  async uploadJson(
    data: Record<string, unknown>,
    name: string = `data-${Date.now()}.json`,
  ): Promise<string | null> {
    if (!this.enabled) {
      this.logger.warn('Pinata no está habilitado');
      return null;
    ...
    try {
      ...
      this.logger.log(`JSON subido a Pinata: ${publicUrl}`);
      return publicUrl;
    } catch (error) {
      const err = error as Error & { response?: { data?: unknown } };
      const details = err.response?.data ?? err.message;
      this.logger.error('Error subiendo JSON a Pinata:', details);
      return null;
    }
  }
