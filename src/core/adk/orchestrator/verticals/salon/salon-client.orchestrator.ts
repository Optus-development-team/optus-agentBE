...
@Injectable()
export class SalonClientOrchestratorService implements OnModuleInit {
  private readonly logger = new Logger(SalonClientOrchestratorService.name);
  ...
  async route(context: RouterMessageContext): Promise<OrchestrationResult> {
    ...
    try {
      ...
      let responseText = '';
      let agentUsed = 'salon_client_orchestrator';
      ...
      this.logger.error(
        `Error en orquestación de cliente salón: ${err.message}`,
      );
      return {
        intent: 'UNKNOWN',
        responseText:
          'Ocurrió un error procesando tu mensaje. Intenta nuevamente en unos momentos.',
        agentUsed: 'salon_client_orchestrator',
      };
    }
  }
  ...
  private initialize(): void {
    ...
    if (!apiKey) {
      throw new Error(
        'Google AI no configurado para SalonClientOrchestratorService',
      );
    }
...
- Toma {app:todayDate} como fecha base para las operaciones.`;
    ...
    this.orchestratorAgent = new LlmAgent({
      name: 'salon_client_orchestrator',
      model,
      instruction,
      description: 'Orquestador de atención al cliente para vertical salón',
      subAgents: [
