...
@Injectable()
export class GeneralClientOrchestratorService implements OnModuleInit {
  private readonly logger = new Logger(GeneralClientOrchestratorService.name);
  ...
  async route(context: RouterMessageContext): Promise<OrchestrationResult> {
    ...
    try {
      ...
      let responseText = '';
      let agentUsed = 'general_client_orchestrator';
      ...
      return {
        ...
        agentUsed: 'general_client_orchestrator',
      };
    }
  }
  ...
  private initialize(): void {
    ...
    if (!apiKey) {
      throw new Error(
        'Google AI no configurado para GeneralClientOrchestratorService',
      );
    }
...
`;
    ...
    this.orchestratorAgent = new LlmAgent({
      name: 'general_client_orchestrator',
      model,
      instruction,
