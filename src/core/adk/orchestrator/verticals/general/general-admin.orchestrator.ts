...
@Injectable()
export class GeneralAdminOrchestratorService implements OnModuleInit {
  private readonly logger = new Logger(GeneralAdminOrchestratorService.name);
  ...
  async route(context: RouterMessageContext): Promise<OrchestrationResult> {
    ...
    if (context.role === UserRole.ADMIN && context.tenant?.companyId) {
      ...
      if (!hasCreds) {
        ...
        return {
          ...
          agentUsed: 'general_admin_orchestrator',
        };
      }
    }
    ...
    try {
      ...
      let responseText = '';
      let agentUsed = 'general_admin_orchestrator';
  ...
  private initialize(): void {
    ...
    if (!apiKey) {
      throw new Error(
        'Google AI no configurado para GeneralAdminOrchestratorService',
      );
    }
...
`;
    ...
    this.orchestratorAgent = new LlmAgent({
      name: 'general_admin_orchestrator',
      model,
      instruction,
